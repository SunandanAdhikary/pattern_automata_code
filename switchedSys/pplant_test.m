...
%          eig_closed=p_unique;
        ...
        if((max(abs(eig_state{i}))<1) && (log(abs(max(eig_state{i})))<exp_decay))
            ifStable(i)= 1;            
...
%         Count(i)= (floor(Taud(i)/sampling_time(i)));     %%%--Permissible self loop count
%         gamma(i)= (Alpha(i)+(log(Mu(i)))/Taud(i));
        gamma_eig(i)= abs(max(pole(closed_loops{i})));
        if(ifStable(i))
    %        gammaStable(p)= (-gamma(i));      %%--gamma_plus
            gammaStable(p)= (-gamma_eig(i));
            p=p+1;
...
alphaVal=Alpha_di';
Gamma=log(gamma_eig');
T=table(samplingTIme,selfLoopCount,minDwellTime,muVal,alphaVal,Gamma)
